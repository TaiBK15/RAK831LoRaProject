...
volatile bool is_send_to_node = false;
    ...
    while(loop) {
        for(int i = 0;i < nb_pkt;i++) {
            ...
            if (packet.status == STAT_CRC_OK) {
                ...
                if (packet.freq_hz == 434500000) {
                    sprintf(data, "{\"channel\": \"%d\", \"payload\": \"%x%x%x%x%x%x\", \"frequency\": %d, \"RSSI\": %f, \"SNR\": %f\n}",
                    chan_num, packet.payload[0], packet.payload[1], packet.payload[2], packet.payload[3], packet.payload[4], packet.payload[5], packet.freq_hz, packet.rssi, packet.snr);    
                }
                else {
                    sprintf(data, "{\"channel\": \"%d\", \"payload\": \"%x%x%x\", \"frequency\": %d, \"RSSI\": %f, \"SNR\": %f\n}",
      ...
      //          sendPacketsToServer(data);
        ...
        while (nb_pkt == 0 && loop == true) {
            nb_pkt = rak831_listen();
            if (is_send_to_node) {
                rak831_send(true, payload_to_node, 0, BW_125KHZ, DR_LORA_SF7, CR_LORA_4_5);
                is_send_to_node = false;
                delay(1);
            }
        }
    }
